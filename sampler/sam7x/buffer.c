...
{
  buffer_info.track = track;
  buffer_info.size = size;
  buffer_info.checksum = checksum;
}

int buffer_write(io_func write_func)
{
  ...
  u32 total = 0;
  u32 checksum = 0;
  u32 size = buffer_info.size;
  uart_send_hex_dword_crlf(size);
  while(size > 0) {
  ...
  // compare checksum
  if(checksum != buffer_info.checksum) {
      return 0x300;
  }
